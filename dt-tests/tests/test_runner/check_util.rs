...
impl CheckUtil {
    pub fn validate_check_log(
        ...
        // check result
        let (expect_miss_logs, expect_diff_logs, expect_extra_logs, _expect_summary_logs) =
            Self::load_check_log(expect_check_log_dir);
        let (actual_miss_logs, actual_diff_logs, actual_extra_logs, actual_summary_logs) =
        ...
        if !actual_summary_logs.is_empty() {
            for log in actual_summary_logs {
                // verify it can be parsed as CheckSummaryLog
                let _: dt_connector::check_log::check_log::CheckSummaryLog =
                    serde_json::from_str(&log).map_err(|e| {
                        anyhow::anyhow!("Failed to parse summary log: {}, error: {}", log, e)
                    })?;
            }
        }
    ...
    pub fn clear_check_log(dst_check_log_dir: &str) {
        let files = ["miss.log", "diff.log", "extra.log", "summary.log"];
        for file in files {
    ...
    fn load_check_log(
        ...
        (miss_logs, diff_logs, extra_logs, summary_logs)
    }
