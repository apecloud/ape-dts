                ...
                if should_finish {
                    ...
                    let partition_col =
                        finish_partition_col
                            .clone()
                            .or_dt_error(DtError::InvariantViolated(
                                "finished PostgreSQL split is missing its partition column"
                                    .to_string(),
                            ))?;
...
impl PgSnapshotDispatchState {
    ...
    fn take_next_pending_work(&mut self) -> anyhow::Result<Option<PgSnapshotWork>> {
        ...
        let work = self
            ...
            .remove(index)
            .or_dt_error(DtError::InvariantViolated(
                "pending PostgreSQL snapshot work is missing".to_string(),
            ))?;
    ...
    fn mark_work_started(&mut self, work: &PgSnapshotWork) -> anyhow::Result<()> {
        ...
        *queued_chunks = queued_chunks
            .checked_sub(1)
            .or_dt_error(DtError::InvariantViolated(
                "PostgreSQL split chunk queued count underflow".to_string(),
            ))?;
        *running_chunks += 1;
        Ok(())
    }
