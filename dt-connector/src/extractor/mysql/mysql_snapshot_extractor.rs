...
impl MysqlSnapshotExtractor {
    ...
    async fn extract_chunk(
        ...
        let query = snapshot_query.create_mysql_query(&tb_meta, values)?;
...
impl MysqlSnapshotDispatchState {
    ...
    fn take_next_pending_work(&mut self) -> anyhow::Result<Option<MysqlSnapshotWork>> {
        ...
        let work = self
            ...
            .remove(index)
            .or_dt_error(DtError::InvariantViolated(
                "pending MySQL snapshot work is missing".to_string(),
            ))?;
    ...
    fn mark_work_started(&mut self, work: &MysqlSnapshotWork) -> anyhow::Result<()> {
        ...
        *queued_chunks = queued_chunks
            .checked_sub(1)
            .or_dt_error(DtError::InvariantViolated(
                "MySQL split chunk queued count underflow".to_string(),
            ))?;
        *running_chunks += 1;
        Ok(())
    }
