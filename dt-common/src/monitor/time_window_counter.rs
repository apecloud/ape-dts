...
impl TimeWindowCounter {
    ...
    pub async fn statistics_in_window(&self, time_window_secs: u64) -> WindowCounterStatistics {
        ...
        for counter in counters.iter() {
            ...
            statistics.max = cmp::max(statistics.max, value);
            statistics.min = cmp::min(statistics.min, value);
...
mod tests {
    ...
    async fn statistics_preserve_sampled_seconds_and_zero_values() {
        ...
        assert_eq!(result.sum, 220);
        assert_eq!(result.count, 4);
        assert_eq!(result.avg_by_count, 55);
        assert_eq!((result.min, result.max), (0, 100));
        assert_eq!(result.avg_by_sec, 73); // (100 + 0 + 120) / 3 sampled seconds
        assert_eq!((result.min_by_sec, result.max_by_sec), (0, 120));
    }
}
