...
impl RedisUtil {
    ...
    pub fn replace_url_address(base_url: &str, host: &str, port: u16) -> anyhow::Result<String> {
        ...
        url.set_host(Some(host)).map_err(|_| {
            DtError::DatabaseInvalidConfig(DbType::Redis, format!("invalid Redis host: {}", host))
        })?;
        url.set_port(Some(port)).map_err(|_| {
            DtError::DatabaseInvalidConfig(DbType::Redis, format!("invalid Redis port: {}", port))
        })?;
        Ok(url.to_string())
    }
    ...
    pub fn get_cluster_master_nodes(
        ...
        if let redis::Value::BulkString(data) = value {
            let nodes_str = String::from_utf8(data).context(DtError::RedisTopology(
                "the Redis CLUSTER NODES response could not be decoded".to_string(),
            ))?;
            let nodes = Self::parse_cluster_nodes(&nodes_str)?;
            let master_nodes = nodes.into_iter().filter(|i| i.is_master).collect();
            Ok(master_nodes)
        } else {
            bail! {DtError::RedisTopology(
                "Redis did not return CLUSTER NODES data".to_string(),
            )}
        }
    }
            ...
            for word in words.iter().skip(8) {
                ...
                let (start, end) = if range.len() > 1 {
                    (
                        range[0]
                            .parse::<u16>()
                            .context(DtError::RedisTopology(format!(
                                "Redis cluster returned an invalid slot range: {word}"
                            )))?,
                        range[1]
                            .parse::<u16>()
                            .context(DtError::RedisTopology(format!(
                    ...
                    let slot_num = word.parse::<u16>().context(DtError::RedisTopology(format!(
                        "Redis cluster returned an invalid slot: {word}"
                    )))?;
                    (slot_num, slot_num)
                };
...
mod tests {
    ...
    fn resolve_connection_config_table() {
        let cases = [
            ...
            ResolveCase {
                name: "verify_ca overrides insecure URL",
                url: "rediss://localhost:6379/0#insecure",
                auth: ssl_auth_with_ca(SslMode::VerifyCa, "/tmp/redis-ca.pem"),
                expected: ResolveExpectation::Resolved {
                    scheme: "rediss",
            ...
            ResolveCase {
                name: "unknown fragment is rejected",
