...
| ssl_mode             | MySQL/PostgreSQL/Redis/MongoDB TLS 模式，可选 `disable`、`require`、`verify_ca`、`verify_full`                                               | verify_ca                                                                                            | 不设置                                                                                 |
| ssl_ca_path          | MySQL/PostgreSQL/Redis/MongoDB TLS 校验使用的 CA 证书路径                            | /etc/ssl/certs/ca.pem                                                                                | 空                                                                                     |
...
- Redis 支持 `disable`、`require`、`verify_ca` 和 `verify_full`。两种校验模式都必须配置 `ssl_ca_path`：`verify_ca` 校验 CA 信任链，`verify_full` 还会校验 URL 主机名/IP 是否匹配证书 SAN。
...
- Redis Cluster 使用两种校验模式时，每个节点都必须提供由配置 CA 签发的证书；`verify_full` 还要求自动发现的各节点主机名/IP 匹配各自证书 SAN。

## MongoDB TLS

- MongoDB 使用 driver 的 rustls 后端，未设置 `ssl_mode` 时沿用 URI 中的 TLS 设置。
- `disable` 使用明文；`require` 仅加密，不校验服务端证书和主机名。
- `verify_ca` 和 `verify_full` 都必须配置 `ssl_ca_path`，并校验服务端证书链及主机名/IP。rustls 后端暂不支持单独关闭主机名校验，因此两种模式目前使用相同实现。
- 显式 SSL 配置覆盖 URI 中的 TLS 设置，同时应用于任务和断点连接。客户端的 `verify_full` 不代表服务端必须通过证书认证客户端。
