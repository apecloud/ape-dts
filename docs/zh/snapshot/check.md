...
MySQL/PostgreSQL/MongoDB snapshot check，抽样发生在抽取阶段。存在元数据行数估算时，
extractor 会把源端读取限制到估算的抽样行数，因此 `sample_rate=25` 大致查询估算行数的
25%。如果估算缺失或为 0，则读取完整源端 stream。对于 inline snapshot check 和 inline CDC
check，`sample_rate=25` 仍会完整写入所有行/变更，然后在目标端 fetch/compare 前只检查 key
hash 落入抽样百分比的行/变更。
