...
以源端结构为基准，校验结果包括 **miss**，**diff** 以及 **summary** 等部分，均以 JSON 的方式呈现。

`miss.log`, `diff.log` 均采用相同的 JSON 结构 (`StructCheckLog`)：
```json
{
  ...
  "dst_sql": "CREATE TABLE `table_name` (id INT PRIMARY KEY)"   // 在 diff 中出现
}
```
